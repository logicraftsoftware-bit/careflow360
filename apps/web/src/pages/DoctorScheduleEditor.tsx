  ...
  return <div className="schedule-page">
    ...
    <div className="schedule-layout">
      <section className="schedule-calendar panel">
        ...
        <div className="calendar-days">{cells.map(date => { const active = schedules.some((item: any) => item.scheduleDate?.slice(0, 10) === isoDate(date) && item.status === "ACTIVE"), outside = date.getMonth() !== month.getMonth(), chosen = selected && isoDate(selected) === isoDate(date); return <button key={isoDate(date)} className={`${active ? "scheduled" : ""} ${outside ? "outside" : ""} ${chosen ? "selected" : ""}`} onClick={() => selectDate(date)}><span>{date.getDate()}</span>{active && <i>Available</i>}</button>; })}</div>
      </section>
