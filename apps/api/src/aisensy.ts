...
export type AiSensySettings = { apiUrl:string; apiKey:string; campaignPaymentPending:string; campaignPaymentSuccess:string; campaignCancelled:string; campaignRescheduled:string;campaignDiagnosticPending:string;campaignDiagnosticSuccess:string;campaignDiagnosticCancelled:string;campaignDiagnosticRescheduled:string;campaignCollectionOtp:string };
...
export const sendCollectionOtp = (a: AppointmentMessage, otp: string) =>
  sendCampaign((settings) => settings.campaignCollectionOtp, a, [otp], undefined, [{ type: "button", sub_type: "url", index: "0", parameters: [{ type: "text", text: otp }] }]);
...
export const sendPaymentPendingMessage=(a:AppointmentMessage,paymentUrl=paymentLinkFor(a.appointmentNumber))=>sendCampaign(s=>s.campaignPaymentPending,a,[a.patientName,a.clinicName,a.doctorName,a.departmentName,indiaDate(a.startsAt),String(a.amount),paymentUrl]);
